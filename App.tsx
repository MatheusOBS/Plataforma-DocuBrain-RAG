...
const App: React.FC = () => {
  ...
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>([
    { role: 'assistant', content: 'DocuBrain Engine v1.5 pronto. Aguardando processamento de consulta semântica.', timestamp: new Date() }
  ]);
  ...
  const generateAutoDoc = async () => {
    ...
    try {
      ...
      setAiDoc("Erro ao conectar com a IA. Por favor, configure sua GEMINI_API_KEY no arquivo .env.local.");
    } finally {
  ...
  const handleExport = (type: string) => {
    ...
    alert(`Exportando esquema em formato ${type}... O download começará em instantes.`);
  };
              ...
              {aiDoc ? (
                ...
                <div className="flex flex-col items-center justify-center py-24 gap-8">
                  <div className="size-24 bg-primary/10 rounded-full flex items-center justify-center animate-bounce">
                    <span className="material-symbols-outlined text-6xl text-primary">model_training</span>
                  </div>
                  <div className="text-center">
                    <p className="text-xl text-gray-500 font-bold">Documentação pronta para processamento.</p>
                    <p className="text-sm text-gray-400 mt-2">Agregando metadados e estruturas de busca semântica...</p>
                  </div>
                  <button onClick={generateAutoDoc} className="bg-primary hover:bg-blue-600 text-white px-10 py-4 rounded-2xl font-black shadow-xl shadow-primary/30 transition-all hover:scale-105 active:scale-95 flex items-center gap-3">
        ...
        return (
          <div className="py-8 space-y-10 animate-in fade-in slide-in-from-left-4 duration-700">
            <div className="flex justify-between items-end">
               <h1 className="text-4xl font-black dark:text-white">Linha do Tempo <span className="text-primary">Git Schema</span></h1>
               <div className="bg-primary/10 text-primary px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest border border-primary/20">3 Revisões Ativas</div>
            </div>
            <div className="bg-white dark:bg-[#121721] border border-gray-200 dark:border-border-dark rounded-[2.5rem] overflow-hidden shadow-2xl">
              <table className="w-full text-left">
                <thead className="bg-gray-50/50 dark:bg-background-dark/50 border-b border-gray-100 dark:border-border-dark">
                  <tr>
                    <th className="px-8 py-6 text-[10px] font-black uppercase tracking-widest text-gray-400">Versão</th>
                    <th className="px-8 py-6 text-[10px] font-black uppercase tracking-widest text-gray-400">Autor Responsável</th>
                    <th className="px-8 py-6 text-[10px] font-black uppercase tracking-widest text-gray-400">Alterações Estruturais</th>
                    <th className="px-8 py-6 text-[10px] font-black uppercase tracking-widest text-gray-400">Data</th>
                    <th className="px-8 py-6 text-[10px] font-black uppercase tracking-widest text-gray-400">Checkpoint</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100 dark:divide-border-dark">
                  {VERSIONS.map(v => (
                    <tr key={v.id} className="hover:bg-gray-50/50 dark:hover:bg-primary/5 transition-all group">
                      <td className="px-8 py-6">
                         <span className="px-3 py-1.5 bg-primary/10 text-primary text-xs font-black rounded-xl border border-primary/10 group-hover:bg-primary group-hover:text-white transition-colors">{v.id}</span>
                      </td>
                      <td className="px-8 py-6 text-sm font-black dark:text-white">{v.author}</td>
                      <td className="px-8 py-6 text-sm font-medium text-gray-500 dark:text-gray-400">{v.changes}</td>
                      <td className="px-8 py-6 text-sm text-gray-400 font-bold tracking-tight">{v.date}</td>
                      <td className="px-8 py-6">
                        <button className="bg-white dark:bg-surface-dark px-4 py-2 border border-gray-200 dark:border-border-dark rounded-xl text-primary hover:bg-primary hover:text-white text-[10px] font-black transition-all shadow-sm">RESTAURAR</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
               ...
               <div className="p-6 border-b border-gray-100 dark:border-border-dark flex justify-between items-center bg-gray-50/50 dark:bg-background-dark/50">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] ml-4 font-black text-gray-400 uppercase tracking-[0.2em] flex items-center gap-2">
                       <span className="material-symbols-outlined text-sm">terminal</span> pgvector console
                    </span>
                  </div>
                  <button onClick={() => alert("Executando Query...")} className="bg-primary hover:bg-blue-600 text-white text-[10px] font-black px-6 py-2 rounded-xl uppercase tracking-widest shadow-lg shadow-primary/20 transition-all hover:scale-105 active:scale-95">Executar</button>
               </div>
               <textarea 
               ...
               <div className="p-6 border-b border-white/5 flex items-center justify-between relative z-10">
                  <div className="flex items-center gap-4">
                    <div className="size-10 bg-primary/20 rounded-2xl flex items-center justify-center">
                      <span className="material-symbols-outlined text-primary text-xl animate-pulse">database</span>
                    </div>
                    <div>
                      <span className="text-xs font-black text-white uppercase tracking-[0.1em]">DocuBrain Analysis Engine</span>
                      <p className="text-[10px] text-green-500 font-bold uppercase tracking-tighter">System Ready - v1.5-Pro</p>
                    </div>
                  </div>
               </div>
               <div className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar relative z-10">
                  {chatMessages.map((m, idx) => (
                    <div key={idx} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-in slide-in-from-bottom-2 duration-300`}>
                      <div className={`max-w-[85%] px-5 py-4 rounded-[1.5rem] text-sm leading-relaxed shadow-xl ${
                        m.role === 'user' 
                        ...
                        <span className="block text-[10px] font-bold opacity-40 mt-2 text-right tracking-tight">{m.timestamp.toLocaleTimeString()}</span>
                      </div>
                    </div>
                  ))}
                  <div ref={chatEndRef} />
        ...
        return (
          <div className="py-8 relative h-[800px] overflow-hidden bg-gray-100/50 dark:bg-background-dark/30 rounded-[3rem] border-2 border-dashed border-gray-200 dark:border-border-dark p-10 animate-in zoom-in-95 duration-700">
             <div className="absolute top-8 left-8 flex gap-3 z-20">
                <div className="bg-white/80 dark:bg-[#121721]/80 backdrop-blur-xl px-5 py-3 rounded-2xl shadow-xl border border-gray-200 dark:border-border-dark flex items-center gap-4 group cursor-pointer hover:border-primary/50 transition-all">
                   <div className="size-8 bg-primary rounded-xl flex items-center justify-center">
                     <span className="material-symbols-outlined text-white text-[20px]">gesture</span>
                   </div>
                   <div>
                     <span className="text-sm font-black dark:text-white block leading-none">Editor Visual</span>
                     <span className="text-[10px] text-green-500 font-black uppercase tracking-tighter">Modo Interativo Ativo</span>
                   </div>
                </div>
             </div>
             ...
             <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-20">
                ...
                <path d="M 280 200 Q 350 250 420 280" stroke="url(#lineGrad)" strokeWidth="3" fill="none" strokeDasharray="8,8" className="animate-[dash_20s_linear_infinite]" />
                <path d="M 640 280 Q 710 210 780 180" stroke="url(#lineGrad)" strokeWidth="3" fill="none" strokeDasharray="8,8" className="animate-[dash_20s_linear_infinite]" />
                <path d="M 520 400 L 520 520" stroke="url(#lineGrad)" strokeWidth="3" fill="none" strokeDasharray="8,8" />
             </svg>
          ...
          <>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-10 mb-14 animate-in fade-in slide-in-from-top-4 duration-700">
              <div className="flex flex-col gap-4">
                <div className="flex items-center gap-3 mb-1">
                  <span className="bg-primary/10 text-primary text-[11px] font-black px-4 py-1.5 rounded-full border border-primary/20 shadow-sm animate-pulse uppercase tracking-[0.2em]">Enterprise Core v1.5.0</span>
                  <div className="size-2 bg-green-500 rounded-full"></div>
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Status: Ready to Scale</span>
                </div>
                <h1 className="text-gray-900 dark:text-white text-6xl font-black leading-tight tracking-tighter">
                  Data <span className="text-primary italic">Intelligence</span> Control
                </h1>
                <p className="text-gray-500 dark:text-[#9da6b9] text-xl font-medium max-w-3xl leading-relaxed">
                  Infraestrutura de alta performance para <span className="text-gray-900 dark:text-white font-bold underline decoration-primary/50 decoration-4 underline-offset-8">LLMs, RAG e Busca Semântica</span> distribuída em nuvem.
                </p>
              </div>
              ...
              <div className="flex gap-4 flex-wrap">
                ...
                >
                  ...
                  <span className={`material-symbols-outlined text-[24px] ${isAnalyzing ? 'animate-spin' : ''}`}>analytics</span>
                  <span>{isAnalyzing ? 'Processando...' : 'Analisar Estrutura'}</span>
                </button>
                <div className="relative group">
                  ...
                  <div className="absolute right-0 top-20 w-64 bg-white dark:bg-[#121721] border border-gray-200 dark:border-border-dark rounded-[2rem] shadow-[0_30px_60px_-10px_rgba(0,0,0,0.3)] py-4 invisible group-hover:visible z-50 transition-all opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 p-2 space-y-1">
                     <button onClick={() => handleExport('Prisma')} className="w-full text-left px-5 py-4 text-xs font-black hover:bg-primary/10 hover:text-primary rounded-2xl transition-all flex items-center justify-between group/item">
                     ...
                     <button onClick={() => handleExport('SQL')} className="w-full text-left px-5 py-4 text-xs font-black hover:bg-primary/10 hover:text-primary rounded-2xl transition-all flex items-center justify-between group/item">
                        <span>PostgreSQL DDL</span>
                        <span className="material-symbols-outlined text-sm opacity-0 group-hover/item:opacity-100 transition-opacity">chevron_right</span>
                     </button>
                     <button onClick={() => handleExport('JSON')} className="w-full text-left px-5 py-4 text-xs font-black hover:bg-primary/10 hover:text-primary rounded-2xl transition-all flex items-center justify-between group/item">
            ...
            <div className="flex items-center justify-between mb-10 mt-16 animate-in fade-in slide-in-from-bottom-2 duration-700 delay-300">
              <div className="flex items-center gap-5">
                 ...
                 <div>
                    <h3 className="text-gray-900 dark:text-white text-3xl font-black tracking-tight leading-none">Mapa de Arquitetura</h3>
                    <p className="text-[10px] text-gray-400 font-black uppercase tracking-[0.2em] mt-2 flex items-center gap-2">
                       <span className="size-1.5 bg-green-500 rounded-full animate-pulse"></span> Prod Cluster: aws-us-east-1
                    </p>
                 </div>
              </div>
              <div className="flex items-center gap-4 bg-gray-100/50 dark:bg-surface-dark/50 p-2 rounded-2xl border border-gray-200 dark:border-border-dark">
                 <button className="bg-white dark:bg-surface-dark shadow-sm border border-gray-200 dark:border-border-dark p-3 rounded-xl text-primary transition-all"><span className="material-symbols-outlined scale-110">grid_view</span></button>
                 <button className="p-3 text-gray-400 hover:text-primary transition-colors"><span className="material-symbols-outlined scale-110">list</span></button>
              </div>
            </div>
  ...
  return (
    <div className="relative flex h-screen w-full flex-row overflow-hidden bg-white dark:bg-background-dark transition-colors duration-500">
      <Sidebar activeItem={activeItem} setActiveItem={setActiveItem} />
      
      <main className="flex-1 flex flex-col h-full overflow-y-auto custom-scrollbar relative">
        <Header />
        <div className="flex-1 px-4 sm:px-10 lg:px-12 py-8 max-w-[1700px] mx-auto w-full">
          {renderContent()}
        </div>
      </main>
    </div>
